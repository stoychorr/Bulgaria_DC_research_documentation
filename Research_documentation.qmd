...
```{r}
options(warn=-1)
plot(seq(1:111),data_stop_isty$diff_GDP, type="l", main="Изменения на диференцираното БВП с два и четири лагови стойности.", xlab="Период в тримесечия", ylab="Изменения в БВП")
```
...
model2 <- lm(GSP_diff ~ capital_diff  + Workers_diff, data = Data)
...
stargazer::stargazer(model,model2 , modeliv, type = "text")
