...
Now we create a separate data set with the differenced data, as outlined above:
...
new_data <- data %>%
  ...
  select(Year, GDP_growth, Birth_rate_growth, Population_growth,Birth_rate)
...
adf_pop <- adf.test(new_data$Population_growth,k=4)
...
grangertest(new_data$GDP_growth~new_data$Population_growth, order=7)
```
...
```{r}
model<-lm(GDP_growth~dplyr::lag(GDP_growth,2), data=new_data)
summary(model)
```
